...
describe('Password Value Object', () => {
  describe('Validation Rules (RN01)', () => {
    test('Should return error if password is empty', () => {
      ...
      expect((sut as InvalidPasswordError).message).toBe('Password is required')
    })

    test('Should return error if password is less than 8 characters', () => {
      const sut = Password.create('Ab1!')
      expect(sut).toBeInstanceOf(InvalidPasswordError)
      expect((sut as InvalidPasswordError).message).toBe('Password must have at least 8 characters')
    })

    test('Should return error if password has no uppercase letter', () => {
      const sut = Password.create('abcdefg1!')
      expect(sut).toBeInstanceOf(InvalidPasswordError)
      expect((sut as InvalidPasswordError).message).toBe('Password must have at least 1 uppercase letter')
    })

    test('Should return error if password has no number', () => {
      const sut = Password.create('Abcdefgh!')
      expect(sut).toBeInstanceOf(InvalidPasswordError)
      expect((sut as InvalidPasswordError).message).toBe('Password must have at least 1 number')
    })

    test('Should return error if password has no special character', () => {
      const sut = Password.create('Abcdefgh1')
      expect(sut).toBeInstanceOf(InvalidPasswordError)
      expect((sut as InvalidPasswordError).message).toBe('Password must have at least 1 special character')
    })

    test('Should create valid password with all requirements met', () => {
      const sut = Password.create('Abcdefgh1!')
      expect(sut).toBeInstanceOf(Password)
      expect((sut as Password).value).toBe('Abcdefgh1!')
    })
