...
describe('LoadCityByIdController', () => {
  ...
  test('Should return 204 if LoadCityById returns undefined', async () => {
    const { sut, loadCityByIdSpy } = makeSut()
  ...
  test('Should return 200 with city if LoadCityById returns a city', async () => {
    ...
    expect(httpResponse).toEqual(ok({
      id: loadCityByIdSpy.result!.id.value,
      name: loadCityByIdSpy.result!.name,
      stateId: loadCityByIdSpy.result!.stateId.value
    }))
  })
